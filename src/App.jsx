...
import { NftFull } from './components/NftFull/NftFull';

function App() {
    const [dataFetch, setDataFetch] = useState({})
    const [searchBy, setSearchBy] = useState('collection')
    ...
    const fetchLoadMore = async () => {
        ...
        if (searchBy === 'collection') {
            url = `tokens/v6?collection=${searchValue.trim()}&limit=6`
        } else if (searchBy === 'user_tokens') {
            url = `users/${searchValue.trim()}/tokens/v7?sortBy=lastAppraisalValue&limit=6`
        }
        const options = {method: 'GET', headers: {accept: '*/*', 'x-api-key': 'demo-api-key'}}
        const response = await fetch(`https://api.reservoir.tools/${url}&continuation=${dataFetch.continuation}`, options)
        const json = await response.json()
        let dataWithLoadMore = {...dataFetch}
        dataWithLoadMore.tokens = [...dataWithLoadMore.tokens, ...json.tokens]
        if (json.continuation) {
            dataWithLoadMore.continuation = json.continuation
        }
        console.log('dataWithLoadMore', dataWithLoadMore)
        setDataFetch(dataWithLoadMore)
    }

    const changeSearchByTo = (params) => {
        if (params === 'collection' || params === 'user_tokens') {
            setSearchBy(params)
        }
    }
    
    const handleNftCardClick = (nftData) => {
    ...
    return (
        <Context.Provider value={{
            searchBy, changeSearchByTo, setSearchValue, setDataFetch
        }}>
            <div className='wrapper'>
                <Navbar showCollections={setShowCollections} />
                <main className='main__block'>
                    {showCollections && <TopCollections />}
                    <SearchForm />
                    <div className='main__block_container'>
                        <div className='main__block_card-list'>
                            {dataFetch.tokens && dataFetch.tokens.map((tokenOne) => (
                                <NftCard nftData={tokenOne} openModal={() => handleNftCardClick(tokenOne)} key={tokenOne.token.tokenId}
                                />
                            ))}
                        </div>
                        {dataFetch.continuation && 
                        <div className='main__block_load-more'>
                            <button
                            ...
                            >
                                {isLoadingLM ?
                                <span className='spinner-loadmore'></span>
                                :
                            ...
                            {/* <span>{dataFetch.continuation}</span> */}
                        </div>}
                    </div>
                </main>
                {modal && 
                <Modal>
                    <NftFull modalNftData={selectedNft} modalIsOpen={setModal}/>
                </Modal>}
                <Footer />
            </div>
        </Context.Provider>
...
// https://api.reservoir.tools/users/0xF296178d553C8Ec21A2fBD2c5dDa8CA9ac905A00/tokens/v7?sortBy=lastAppraisalValue&limit=6
// https://api.reservoir.tools/tokens/v6?collection=0x8d04a8c79ceb0889bdd12acdf3fa9d207ed3ff63&limit=6